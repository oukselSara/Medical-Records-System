...
export default function Patients() {
 ...
 const handleExportPDF = async (patient: Patient) => {
  try {
    ...
    // Fetch patient's prescriptions and treatments
    const [prescriptions, treatments] = await Promise.all([
      prescriptionsCollection.getByPatient(patient.id).catch(() => []),
      treatmentsCollection.getByPatient(patient.id).catch(() => []),
    ]);
    ...
    toast({ 
      ...
      description: "The patient record has been downloaded."
    });
  } catch (error) {
